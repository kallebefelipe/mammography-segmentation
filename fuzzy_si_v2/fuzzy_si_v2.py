...
for a in beta:
	for b in tol:
		for c in mode:
		 	worksheet.write(0, col,' beta='+str(a)+' tol='+str(b)+' mode='+str(c)+' ')
		 	col = col+8
...
for i in images:
	...
	for x in range(0,image.shape[0]):	
	 	for y in range(0,image.shape[1]):
	 		...
	 		imageShape[x,y] = math.exp( ((-1)*math.pow((x_n-xm),2))/(2*constantex*desviox)  +   ((-1)*math.pow((y_n-ym),2))/(2*constantey*desvioy) )   #calculo de f(x,y)
	...
	for a in beta:
 		for b in tol:
 			for c in mode:
			 	...
			 	if somaTN == 0 or (somaFP + somaTN) == 0:
			 		Specificity = 0
			 	else:
			 		Specificity = somaTN /(somaFP + somaTN)
			 	if (somaTP + somaFN + somaFP) ==0:
			 		Jaccard = 0
			 	else:
